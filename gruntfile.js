...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    uglify: {
      ...
      dist: {
        files: {
          '<%= _dest %>/js/searxng.head.min.js': ['src/js/head/*.js'],
          '<%= _dest %>/js/searxng.min.js': [
            'src/js/main/*.js',
        ...
        files: [
          {
            ...
            dest: '<%= _dest %>/css/searxng.min.css',
            nonull: true,
            filter: file_exists,
          },
          {
            src: ['src/less/style-rtl.less'],
            dest: '<%= _dest %>/css/searxng-rtl.min.css',
            nonull: true,
            filter: file_exists,
          },
        ],
    ...
    image: {
      svg4web: {
        options: {
          svgo: ['--config', 'utils/svg4web.svgo.js']
        },
        files: {
          '<%= _templates %>/searxng-wordmark.min.svg': 'src/svg/searxng-wordmark.svg',
          '<%= _dest %>/img/searxng.svg': 'src/svg/searxng.svg',
          '<%= _dest %>/img/img_load_error.svg': 'src/svg/img_load_error.svg'
        }
      },
      favicon: {
        options: {
          svgo: ['--config', 'utils/svg4favicon.svgo.js']
        },
        files: {
          '<%= _dest %>/img/favicon.svg': 'src/svg/searxng-wordmark.svg'
        }
      },
    },
    svg2png: {
      favicon: {
        files: {
          '<%= _dest %>/img/favicon.png': 'src/svg/searxng-wordmark.svg',
          '<%= _dest %>/img/searxng.png': 'src/svg/searxng.svg',
        }
      }
    },
    svg2jinja: {
  ...
  grunt.registerMultiTask('svg2jinja', 'Create Jinja2 macro', function () {
    ...
    const template = `{# this file was generated by gruntfile.js #}
{%- set icons = {
